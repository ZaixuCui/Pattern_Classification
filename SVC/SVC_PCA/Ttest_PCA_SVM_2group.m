function Accuracy = Ttest_PCA_SVM_2group(Subjects_Data, Subjects_Label, P_Value, Pre_Method, ResultantFolder, RetainQuantity)
%
% Subject_Data:
%           m*n matrix
...
for i = 1:Subjects_Quantity
    ...
    All_Training_New_PCA = double(All_Training_New_PCA);
    model = svmtrain(Label, All_Training_New_PCA,'-t 0');
    ...
    % predicts
    test_data_New_PCA = double(test_data_New_PCA);
    [predicted_labels(i), ~, tmp] = svmpredict(test_label, test_data_New_PCA, model);
    ...
    decision_values(i) = w * test_data_New_PCA' - model.rho;
    
    % For averaging w
    w2{i} = zeros(1, Features_Quantity);
    for j = 1 : model.totalSV
        tmp = full(model.sv_coef(j) * model.SVs(j, :));
        w2{i}(RetainID{i}) = w2{i}(RetainID{i}) + tmp * COEFF';
    end
