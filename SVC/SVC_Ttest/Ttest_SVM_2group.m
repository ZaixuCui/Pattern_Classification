function Accuracy = Ttest_SVM_2group(Subjects_Data, Subjects_Label, P_Value, Pre_Method, ResultantFolder, RetainQuantity)
%
% Subject_Data:
%           m*n matrix
...
for i = 1:Subjects_Quantity
    ...
    [predicted_labels(i), ~, tmp] = svmpredict(test_label, test_data_New, model);
    
    % Calculate decision value
    w{i} = zeros(size(model.SVs(1, :)));
    for j = 1 : model.totalSV
        w{i} = w{i} + model.sv_coef(j) * model.SVs(j, :);
    end
    decision_values(i) = w{i} * test_data_New' - model.rho;
    
    % For averaging w
    w2{i} = zeros(1, Features_Quantity);
    for j = 1 : model.totalSV
        w2{i}(RetainID{i}) = w2{i}(RetainID{i}) + model.sv_coef(j) * model.SVs(j, :);
    end
