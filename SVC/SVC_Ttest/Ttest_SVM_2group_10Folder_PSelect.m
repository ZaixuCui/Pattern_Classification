function [Accuracy Sensitivity Specificity] = Ttest_SVM_2group_10Folder_PSelect(Subjects_Data, Subjects_Label, P_Value_Range, Pre_Method, ResultantFolder)
%
% Subject_Data:
%           m*n matrix
...
for i = 1:10
    ...
    for j = 1:length(P_Value_Range)
        Accuracy(j) = Ttest_SVM_2group_10Folder_ACC(Training_all_data, Label, P_Value_Range(j), Pre_Method);
    end
...
%     end
%     decision_values_tmp = w * test_data' - model(i).rho;
