function [Accuracy, Sensitivity, Specificity] = SVM_2group_NFolds_CSelect(Subjects_Data, Subjects_Label, Fold_Quantity, CVRepeatTimes, C_Range, Pre_Method, ResultantFolder)
%
% Subject_Data:
%           m*n matrix
...
for t = 1:CVRepeatTimes
  ...
  for i = 1:Fold_Quantity
    ...
    for m = 1:length(C_Range)
      for k = 1:20
        Accuracy_tmp(k) = SVM_2group_NFolds(Training_all_data, Label, Fold_Quantity, C_Range(m), Pre_Method);
      end
    ...
    model(i) = svmtrain(Label, Training_all_data, ['-t 0 -c ' num2str(C_Optimal)]);
  ...
  ResultantFolder_I = [ResultantFolder '/Time_' num2str(t)];
  save([ResultantFolder_I filesep 'Y.mat'], 'Y_group1', 'Y_group0');
  save([ResultantFolder_I filesep 'Category.mat'], 'Category_group1', 'Category_group0');
  ...
  save([ResultantFolder_I filesep 'WrongInfo.mat'], 'group0_Wrong_Quantity', 'group0_Wrong_ID', 'group1_Wrong_Quantity', 'group1_Wrong_ID');
  ...
  save([ResultantFolder_I filesep 'Accuracy.mat'], 'Accuracy');
  ...
  save([ResultantFolder_I filesep 'Specificity.mat'], 'Specificity');
  % save contribution map
  save([ResultantFolder_I filesep 'w_Brain.mat'], 'w');
