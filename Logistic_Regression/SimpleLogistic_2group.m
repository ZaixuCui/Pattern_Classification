function Accuracy = SimpleLogistic_2group(Subjects_Data, Subjects_Label, Num_BoostingIterations, Pre_Method, ResultantFolder)
%
% Subject_Data:
%           m*n matrix
...
t = weka.classifiers.functions.SimpleLogistic();
% tmp=wekaArgumentString({'-S'});
% tmp=wekaArgumentString({'-F', 2});
tmp=wekaArgumentString({'-I', Num_BoostingIterations});
t.setOptions(tmp);
