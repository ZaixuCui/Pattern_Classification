...
function w_Brain = W_Calculate_PCA_LDA(Subjects_Data, Subjects_Label, type, ResultantFolder, Parameters)
...
model_All = LDA_train(PCA_All, Subjects_Label);
New_w = zeros(1, length(latent));
New_w(1:length(model_All.w)) = New_w(1:length(model_All.w)) + model_All.w';
w_Brain = New_w * COEFF';
