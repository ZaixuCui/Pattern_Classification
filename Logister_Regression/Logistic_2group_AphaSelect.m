function Logistic_2group_AphaSelect(Subjects_Data, Subjects_Label, alpha_range, Pre_Method, ResultantFolder)
%
% Subject_Data:
%           m*n matrix
...
for i = 1:Subjects_Quantity
    ...
    for j = 1:length(alpha_range)
        Acc = Logistic_2group_AphaSelect_Child(Training_all_data, Label, alpha_range(j), Pre_Method);
        if Acc > Max_Acc
            Best_alpha = alpha_range(j);
            Max_Acc = Acc;
        end
    end
    ...
    theta = Logistic_train(Training_all_data, Label, Best_alpha);
    ...
    save([ResultantFolder filesep 'predicted_labels.mat'], 'predicted_labels');
